...
var schema = z.object({
  ...
  config: z.record(z.string(), z.unknown()).optional().describe("Automation configuration object (required for create and update)")
});
try {
  const jsonSchema = zodToJsonSchema(schema);
  console.log(JSON.stringify(jsonSchema, null, 2));
  const verify = (obj, path = []) => {
    if (!obj) return true;
